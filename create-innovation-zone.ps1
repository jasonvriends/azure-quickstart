...
$AzureResourceGroups="desktops,guacamole,networking,automation"
...
New-AzResourceGroupDeployment -ErrorVariable deployError -ErrorAction SilentlyContinue -Name "deploy-vnet" -ResourceGroupName $azureNetworkingRG -TemplateFile $ScriptPath/3-virtual-network/azuredeploy.json -TemplateParameterFile $ScriptPath/3-virtual-network/azuredeploy.parameters.json -externalIp $ExternalIp
...
New-AzResourceGroupDeployment -ErrorVariable deployError -ErrorAction SilentlyContinue -AsJob -Force -Name "deploy-guac" -ResourceGroupName $azureGuacamoleRG -TemplateFile $ScriptPath/5-apache-guacamole/azuredeploy.json -TemplateParameterFile $ScriptPath/5-apache-guacamole/azuredeploy.parameters.json -pwdOrPsk $securepwdOrPsk -mysqlRootPwd $securemysqlRootPwd -dbUserPwd $securedbUserPwd -Size Standard_D2s_v3
...
New-AzResourceGroupDeployment -ErrorVariable deployError -ErrorAction SilentlyContinue -AsJob -Force -Name "deploy-desktop" -ResourceGroupName $AzureDesktopsRG -TemplateFile $ScriptPath/4-virtual-machines/azuredeploy.json -TemplateParameterFile $ScriptPath/4-virtual-machines/azuredeploy.parameters.json -pwdOrPsk $securepwdOrPsk -dnsLabelPrefix "clouddesktop" -Size "Standard_D2s_v3" -osPublisher "MicrosoftWindowsServer" -osOffer "WindowsServer" -osSKU "2019-Datacenter"
...
& "$ScriptPath/2-scaffold/apply-scaffold.ps1" -Region $AzureRegion -AutomationRG $AzureAutomationRG -Environment $AzureEnvironment -SubscriptionId $AzureSubscriptionId -TimeZone $PowerManagementTimeZone -NetworkingRG $AzureNetworkingRG
